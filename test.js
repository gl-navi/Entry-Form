(function() {
    ...
    if (!oldDiv) {
        ...
        return; // FIX #7: actually stop on missing target
    }
    ...
    styleElement.textContent = `
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* FIX #16: tooltip centered, wraps on small screens */
        .tooltip-icon::after {
    ...
    // Form HTML (FIX #14: rel="noopener noreferrer", FIX #18: aria-describedby on checkbox)
    ...
    // ===== FIX #1: Marketo wait with hard timeout =====
    ...
    // ===== FIX #3 & #4: fetch with timeout, no retry on TypeError/AbortError =====
    ...
    form.addEventListener('submit', function(event) {
        ...
        // FIX #5: real duplicate-submit guard (module scope)
        if (isSubmissionInProgress) return;

        // FIX #17: track every submit attempt, not only valid ones
        ...
        // FIX #11: ensure timestamp is set on submit if checkbox is checked
        ...
        .then(response => {
            ...
            // ===== FIX #2: Marketo submission with null check + try/catch =====
...
ご確認のほどよろしくお願いいたします。`;
            ...
            if (!error.status && (error.name === 'TypeError' || error.name === 'AbortError')) {
                ...
                // FIX #15: Object.values instead of for...in
            ...
            // FIX #5: fallbackShown is module-scoped so it can't double-render
    ...
    // ===== Real-time validation (FIX #6: input.id, not this.id) =====
    ...
    function validateEmail() {
        ...
        // FIX #9: accept if EITHER regex matches (was: both required)
    ...
    // FIX #8: don't call validateEmail() — handle confirmation independently
    ...
    function validatePhone() {
        ...
        // FIX #9: accept if EITHER regex matches
